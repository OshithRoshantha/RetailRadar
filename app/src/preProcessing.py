...
from src.insights import customerInsights, geographicInsights, salesInsights
...
def initialProcessing():
    ...
    df = df.drop('Name', 'Email', 'Phone', 'Address', 'State', 'Zipcode', 'Product_Brand', 'Product_Type', 'Feedback', 'Ratings', 'products')
    df = df.withColumn(
        "Date",
        F.coalesce(
